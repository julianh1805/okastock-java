...
class ProduitServiceTest {
    ...
    private Utilisateur utilisateur = new Utilisateur(UUID.fromString(utilisateurId), "Test", 12345678910111L, 44300, 666666666L, "http://www.test.com", "-", true, "test@test.com", "1234AZER", new ArrayList<>(), new ArrayList<>());
    ...
    void itShouldAdd() {
        ...
        //When
        underTest.create(produitToAdd, categorieName, utilisateurId);
        //Then
    ...
    @Test
    @WithMockCustomUser(principal = "bad_user_id")
    void itShouldThrownAnExeceptionIfUserIdNotEqualsWhenUpdate() {
        ...
        assertThatThrownBy(() -> underTest.update(produit, "meubles"))
                ...
                .hasMessageContaining("Vous n'avez pas les droits pour effectuer cette action.");
    }
    ...
    void itShouldThrownAnExeceptionIfNotGetByIdWhenUpdate() {
        ...
        UUID fakeProduitId = UUID.randomUUID();
        String categorieName = "meubles";
        Produit produitToUpdate = new Produit(fakeProduitId, "Titre", "Petite description", new BigDecimal("10.27"), 8, null, utilisateur);
